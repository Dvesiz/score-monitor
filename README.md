...
| **Bark 推送** | 新成绩/成绩更新时通过 Bark 推送到 iOS，没有输入Bark、Pushplus等不会推送，但是程序内也能看到成绩 |
| **持续监控** | 默认每 5 分钟检查一轮 |
| **多账号** | 支持命令行参数临时切换账号 |
...
> **使用前提：** 本程序仅能在校园内网环境中访问教务系统。校外使用前，请先登录学校 VPN，并保持 VPN 连接。
...
> ⚠️ **密码第一次必须输入正确**。登录失败会自动重试（验证码识别错误等原因），但重试 5 次后账户可能被锁定。建议先手动在浏览器确认密码正确再使用本程序。
